...
The hypercube enables you to calculate **measures** (like sales, revenue, or user counts) across any combination of these dimensions, ensuring your calculations are always consistent and reusable. For example, you can analyze:

- *Total Sales* by **Month**
- *Total Sales* by **Month** and **Product Category**
- *Total Sales* by **Month**, **Product Category**, and **Region**

---

### How Cube Alchemy Builds the Hypercube

In Cube Alchemy, the hypercube is not limited to a single table. It intelligently combines **multiple DataFrames** into a unified, logical model that you can query seamlessly. You don't need to write manual joins; the hypercube handles the relationships for you.
...
>
> For the hypercube to function correctly, your data model must be a **Directed Acyclic Graph (DAG)**. This means you must avoid **cyclic relationships** (or circular references), where a path of relationships leads back to the starting table. Such cycles break the logic of data traversal and can lead to incorrect or infinite aggregations.
