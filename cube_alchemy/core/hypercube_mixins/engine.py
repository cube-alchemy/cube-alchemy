...
class Engine(GraphVisualizer, Filter, Query):
    ...
    def _join_trajectory_keys(self, trajectory: List[str]) -> pd.DataFrame:
        ...
        """
        def key_and_index_cols(df: pd.DataFrame) -> List[str]:
        ...
        if not trajectory:
            ...
            if len(base_tables) == 1:
                t = base_tables[0]
                cols = key_and_index_cols(self.tables[t])
    ...
    def relationship_matrix(self, context_state_name: str = 'Unfiltered', core = False) -> pd.DataFrame:
        ...
        return self.dimensions(columns_to_fetch=shared_columns, context_state_name=context_state_name)
        ...
        for key in shared_keys:
            ...
            for i in range(len(tables_sharing_key)):
                for j in range(i + 1, len(tables_sharing_key)):
                    ...
                    s1 = self.dimensions([f'_index_{t1}', shared_column], context_state_name=context_state_name).dropna()[shared_column].dropna()
                    s2 = self.dimensions([f'_index_{t2}', shared_column], context_state_name=context_state_name).dropna()[shared_column].dropna()
